...
// curated tag palette — deliberately a different hue family from the
// amber/sage/dim priority colors, so tag chips never compete with priority stripes
const TAG_PALETTE = [
  "#6b8fb5", // steel blue
  "#a78bc4", // lavender
  "#c47b8b", // dusty rose
...
export default function TagsView() {
  ...
  const saveAdd = () => {
    ...
    setTags((prev) => [...prev, { id: Date.now(), name: addName.trim(), color: addColor }]);
    setAdding(false);
  };
  ...
  return (
    ...
    >
      <style>{`
        ...
        .tag-row { cursor: pointer; transition: background 0.12s ease; }
        .tag-row:active { background: ${COLORS.panel}; }
      ...
      <div style={{ width: "100%", maxWidth: "420px", padding: "0 0 100px 0" }}>
        ...
        <div style={{ padding: "28px 20px 16px", borderBottom: `1px solid ${COLORS.border}` }}>
          ...
          <div style={{ fontSize: "12px", color: COLORS.dim, marginTop: "4px" }}>{tags.length} defined</div>
        </div>
